...
function App() {
  ...
  useEffect(() => {
    ...
    const savedUser = localStorage.getItem('pokedex_session');
    if (savedUser) {
  ...
  const handleLogin = (user) => {
    setCurrentUser(user);
    localStorage.setItem('pokedex_session', JSON.stringify(user));
  };
        ...
        <div className="w-full px-4 md:px-8 text-center flex flex-col items-center justify-center gap-6">
          <div className="w-full flex flex-col md:flex-row items-center justify-between gap-4 max-w-[1200px]">
            <div className="md:w-1/4 flex justify-start">
              {currentUser && (
                <div className="flex flex-col items-start bg-blue-900/20 px-4 py-2 rounded-2xl border border-blue-500/20">
                  <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Entrenador</span>
                  <span className="text-xs font-bold text-white truncate max-w-[150px]">{currentUser.email}</span>
              ...
              {currentUser && (
                <div className="relative w-full max-w-[300px]">
                  <input
                    ...
                    className="w-full bg-[#1a1a2e] border-2 border-blue-900/50 rounded-full py-2 px-6 pl-12 text-sm font-bold text-white focus:border-yellow-400 focus:outline-none transition-all"
                  />
          ...
          <>
            {loading && !selectedPokemon && (
              <div className="flex flex-col items-center justify-center py-24">
                ...
                <p className="mt-6 text-xl font-black text-yellow-100 uppercase tracking-widest animate-pulse">Scanning...</p>
              </div>
            )}
                ...
                {selectedPokemon ? (
                  ...
                  !loading && (
                    <div className="animate-fadeIn w-full">
                      ...
                      <PokemonList pokemons={filteredPokemons} onSelect={handleSelectPokemon} currentUser={currentUser} />
                    </div>
                  )
                )}
              </div>
